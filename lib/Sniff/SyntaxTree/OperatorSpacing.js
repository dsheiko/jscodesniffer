	...
	Sniff = function( sourceCode, mediator ) {
		...
		return {
			...
			*/
			run: function( rule, node ) {
				...
				if ( ( node.type === "VariableDeclarator" ) &&
					...
					that.sniff( node.id, node.init, lPos, rPos,
				...
				if ( ( node.type === "AssignmentExpression" || node.type === "BinaryExpression" ) &&
					...
					that.sniff( node.left, node.right, lPos, rPos,
						rule.allowOperatorPrecedingWhitespaces, "OperatorPrecedingWhitespaces" );
			...
			/**
			* Find relative position of previous non-whitespace character
			* @access protected
			* @param {number} lPos
			* @param {number} rPos
			* @returns {number}
			*/
			...
			/**
			* Find relative position of next non-whitespace character
			* @access protected
			* @param {number} lPos
			* @param {number} rPos
			* @returns {number}
			...
			*/
			sniff: function( lNode, rNode, lPos, rPos, expected, errorCode ) {
				...
				if ( fragment.find( "\n" ) === -1 ) {
					if ( fragment.length() !== expected ) {
						mediator.publish( "violation", NAME, errorCode, [ lPos, rPos ], {
							...
							expected: expected,
							content: sourceCode.extract( lPos - 1, rPos + 1 )
						});
					}
				}
			}
