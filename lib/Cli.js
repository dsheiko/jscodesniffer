		...
		return {
			...
			*/
			applyToFile: function( pathArg, fn ) {
				if ( this.ignoreList.length && this.ignoreList.indexOf( pathArg ) !== -1 ) {
					console.log( " * `" + pathContainer.relative( process.cwd(), pathArg ) +
						"` ignored in concordance with .jscsignore" );
					return;
				}
				this.fileCount++;
			...
			/**
			* https://github.com/dsheiko/jscodesniffer/issues/22
			* @param {String} pathArg
			* @returns {void}
			*/
			readIgnoreList: function( pathArg ) {
				var LOCAL_CFG_FILE = ".jscsignore",
						that = this,
				...
				glob = require( "glob" );
				list = fsContainer.readFileSync( cfgPath, "utf-8" );
				list.split( "\n" ).forEach(function( rule ){
					var files;
					if ( !rule ) {
						return;
					}
					files = glob.sync( rule );
					if ( files ) {
						that.ignoreList = that.ignoreList.concat( files.map(function( file ){
							return pathContainer.resolve( file );
						}));
					}
				});
			},
